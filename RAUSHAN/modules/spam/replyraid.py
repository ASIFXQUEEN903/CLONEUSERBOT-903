...
async def pornspam(xspam: Client, e: Message): 
    ...
    if int(e.chat.id) in GROUP:
         return await e.reply_text("**Qᴜᴇᴇɴ ᴍᴀɴᴀ ᴋᴀʀɪᴄʜɪ ᴇɪ ɢʀᴏᴜᴘ ʀᴇ ꜱᴘᴀᴍ ʜᴀʙᴀɴɪ ᴍᴀɴᴇ ʜᴀʙᴀɴɪ ᴀᴅʜɪᴋᴀ ɢ ᴍᴀꜱᴛᴀɴɪ ᴋᴀʀᴇɴɪ**")
    kkk = "**#Pornspam**"
    count = int(counts)
...
async def raid(xspam: Client, e: Message):  
      ...
      if len(RAUSHAN) == 2:
          ...
          if int(e.chat.id) in GROUP:
               return await e.reply_text("**QUEEN PACHARE ACHII TARA KICHI KARIHABANI YARA 👸.**")
...
async def gmute_user(client: Client, message: Message):
    ...
    try:
        if user.id not in (await get_rraid_users()):
           await ex.edit("QUEEN PACHARE ACHII TARA KICHI KARIHABANI YARA 👸")
           return
